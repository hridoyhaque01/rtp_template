...
import { useSendForgotEmailMutation } from "../features/auth/authApi";
...
function ForgotPassword() {
  const [sendForgotEmail, { isLoading }] = useSendForgotEmailMutation();
  ...
  const handleSubmit = (event) => {
    ...
    if (!email) {
      ...
      sendForgotEmail(formData)
        .unwrap()
        .then((res) => {
          navigate(userRouteLinks.resetPassword.path, {
            state: {
              payload: email,
            },
          });
